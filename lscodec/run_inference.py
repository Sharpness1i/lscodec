...
from .models import loaders, lscodecModel, LMModel, LMGen
...
class InferenceState:
    lscodec: lscodecModel
    text_tokenizer: sentencepiece.SentencePieceProcessor
    lm_gen: LMGen

    def __init__(self, model_type: str, lscodec: lscodecModel, text_tokenizer: sentencepiece.SentencePieceProcessor,
                 lm: LMModel, batch_size: int, cfg_coef: float, device: str | torch.device, **kwargs):
        self.model_type = model_type
        self.lscodec = lscodec
        ...
        self.device = device
        self.frame_size = int(self.lscodec.sample_rate / self.lscodec.frame_rate)
        self.batch_size = batch_size
        self.lscodec.streaming_forever(batch_size)
    ...
    def run(self, in_pcms: torch.Tensor) -> list[tuple[torch.Tensor, torch.Tensor]]:
        ...
        while not all(eos_reached):
            if chunks:
                ...
                codes = self.lscodec.encode(chunk)
            else:
                if self.model_type == 'hibiki':
                    if need_eos_input:
                        ...
                        eos_value = self.lscodec.cardinality
                        codes = torch.full(
                            (self.batch_size, self.lscodec.num_codebooks, 1),
                            eos_value, device=device, dtype=torch.long)
                    else:
                        silence = torch.zeros((self.batch_size, self.lscodec.channels, self.frame_size), device=device)
                        codes = self.lscodec.encode(silence)
                else:
            ...
            out_pcm = self.lscodec.decode(tokens[:, 1:]).cpu()
...
def main():
    ...
    parser.add_argument("--lscodec-weight", type=str, help="Path to a local checkpoint file for lscodec.")
    parser.add_argument("--lscodec-weight", type=str, help="Path to a local checkpoint file for lscodec.")
    parser.add_argument("--hf-repo", type=str, default=loaders.DEFAULT_REPO,
                        help="HF repo to look into, defaults lscodecko. "
    ...
    checkpoint_info = loaders.CheckpointInfo.from_hf_repo(
        args.hf_repo, args.lscodec_weight, args.lscodec_weight, args.tokenizer, args.config)
    log("info", "loading lscodec")
    lscodec = checkpoint_info.get_lscodec(device=args.device)
    log("info", "lscodec loaded")
    text_tokenizer = checkpoint_info.get_text_tokenizer()
    log("info", "loading lscodec")
    lm = checkpoint_info.get_lscodec(device=args.device, dtype=args.dtype)
    log("info", "lscodec loaded")

    log("info", f"loading input file {args.infile}")
    in_pcms, _ = sphn.read(args.infile, sample_rate=lscodec.sample_rate)
    in_pcms = torch.from_numpy(in_pcms).to(device=args.device)
    in_pcms = in_pcms[None, 0:1].expand(args.batch_size, -1, -1)

    state = InferenceState(
        checkpoint_info.model_type, lscodec, text_tokenizer, lm,
    ...
    if args.outfile:
        ...
        for index, (_, out_pcm) in enumerate(out_items):
            if len(out_items) > 1:
                ...
                outfile_ = outfile
            duration = out_pcm.shape[1] / lscodec.sample_rate
            log("info", f"writing {outfile_} with duration {duration:.1f} sec.")
            sphn.write_wav(str(outfile_), out_pcm[0].numpy(), sample_rate=lscodec.sample_rate)
