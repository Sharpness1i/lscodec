...
class ServerState:
    ...
    def __init__(self, model_type: str, lscodec: lscodecModel, text_tokenizer: sentencepiece.SentencePieceProcessor,
        ...
        self.lscodec.streaming_forever(1)
    ...
    def warmup(self):
        for chunk in range(4):
            ...
            for c in range(codes.shape[-1]):
                ...
                if tokens is None:
                    continue
                _ = self.lscodec.decode(tokens[:, 1:])
    ...
    async def handle_chat(self, request):
            ...
            while True:
                ...
                while all_pcm_data.shape[-1] >= self.frame_size:
                    ...
                    if skip_frames:
                        ...
                        # view of the model it is in the past. We still `lscodec.encode` for simplicity,
                    ...
                    for c in range(codes.shape[-1]):
                        ...
                        main_pcm = self.lscodec.decode(tokens[:, 1:])
        ...
        async with self.lock:
            opus_writer = sphn.OpusStreamWriter(self.lscodec.sample_rate)
            opus_reader = sphn.OpusStreamReader(self.lscodec.sample_rate)
            self.lscodec.reset_streaming()
...
def main():
    ...
    checkpoint_info = loaders.CheckpointInfo.from_hf_repo(
        args.hf_repo, args.lscodec_weight, args.lscodec_weight, args.tokenizer,
    ...
    log("info", "loading lscodec")
    lm = checkpoint_info.get_lscodec(device=args.device, dtype=args.dtype, fuse_lora=args.fuse_lora)
    log("info", "lscodec loaded")

    state = ServerState(checkpoint_info.model_type, lscodec, text_tokenizer, lm, args.cfg_coef, args.device,
    ...
    if args.static is None:
        ...
        dist_tgz = hf_hub_download("kyutai/lscodec-artifacts", "dist.tgz")
