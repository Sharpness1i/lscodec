...
class TrainDataLoadIter:
    ...
    def load_wav(self, info: str, fs=None, cut_duration=None, return_both=True):

        fs_target_24 = 24000
        wav_duration = librosa.get_duration(path=info)
        ...
        if wav_24k.ndim == 1:
            wav_24k = wav_24k[None, :]
        else:
            wav_24k = wav_24k[:1, :]

        if cut_duration is not None:
            wav_24k, _ = self.pad_or_cut_wav(wav_24k, length=int(cut_duration * fs_target_24))

        # ---- 降采样到 16kHz ----
        fs_target_16 = 16000
        wav_16k = librosa.resample(
            wav_24k[0], orig_sr=fs_target_24, target_sr=fs_target_16
        )[None, :]

        if return_both:
            return wav_24k, wav_16k, fs_target_24, fs_target_16
        else:
            return wav_16k, fs_target_16

    
    def load_wav_queue(self, info, fs, cut_duration, q):
        try:
            wav_24k, wav_16k, fs_target_24, fs_target_16 = self.load_wav(info, fs, cut_duration)
            q.put((wav_24k, wav_16k, fs_target_24, fs_target_16))
    ...
    def process_one_sample(self, fs, cut_duration):
        ...
        try:
            wav_path = os.path.join(self.speech_scp_base_dir,wav_path)
            wav_24k, wav_16k, fs_target_24, fs_target_16 = self.load_wav_with_timeout(wav_path, fs, cut_duration, timeout=2000.0)
        except Exception as e:
            print(e)
            return self.process_one_sample(fs, cut_duration)
        
        length_16k = wav_16k.shape[-1]
        length_24k = wav_24k.shape[-1]
        wav_16k = self.normalize_wav(wav_16k)
        wav_24k = self.normalize_wav(wav_24k)

        return wav_16k,wav_24k,length_16k,length_24k

    def data_iter_fn(self, q, event):
        executor = ThreadPoolExecutor(max_workers=self.num_workers)
        for _ in range(len(self)): # for each batch
            fs = 24000 # sample a fs
            ...
            batch_wav_16k = torch.from_numpy(np.concatenate(batch_wav_16k, axis=0)).float()
            batch_wav_24k = torch.from_numpy(np.concatenate(batch_wav_24k, axis=0)).float()

            lengths_16k = torch.LongTensor(lengths_16k)
            lengths_24k = torch.LongTensor(lengths_24k)
            
            q.put((batch_wav_16k, batch_wav_24k, lengths_16k,lengths_24k))
        event.set()
...
class ValDataLoadIter:
    def __init__(
        ...
        num_workers: int = 1, 
        prefetch: int = 0,
        speech_scp_base_dir:Union[str, Path]=None,
